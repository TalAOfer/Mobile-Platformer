...
public class Tweener : MonoBehaviour
{
    ...
    {
        ResetAnimations();
        return transform.DOShakeScale(blueprint.shakeDuration, blueprint.shakeStrength, blueprint.shakeVibrato, blueprint.shakeRandomness, false)
            .SetUpdate(true)
            .SetEase(blueprint.ease);
    }
    ...
    private void ResetAnimations()
    {
        ...
        if (resetPos) transform.localPosition = originalPosition; // Reset to original position
        if (resetScale) transform.localScale = originalScale; // Reset to original scale
    }
